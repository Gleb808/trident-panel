...
function notice() {
  ...
  if (!st.connected) return `<div class="notice"><span>◷</span><div><strong>Агент сервера не подключён</strong><br>Конфиги сохраняются в панели. Применение и автоматическое отключение по сроку начнут работать после запуска агента на VPS.</div></div>`;
  if (!st.synced) return `<div class="notice"><span>◷</span><div><strong>Ожидается применение изменений</strong><br>${esc(st.agent?.message || 'Агент обновит три сервиса.')} Старые подключения могут оставаться активными до успешного применения.</div></div>`;
...
function protocols() {
  ...
  return `<div class="protocols">${[['naive','NaiveProxy','N',s.naivePort, s.naiveQuic ? 'TCP + UDP' : 'TCP','HTTPS · Caddy'],['mieru','mieru','M',`${s.mieruStart}–${s.mieruEnd}`,'TCP','Шифрованный транспорт · mita'],['vless','VLESS','V',s.vlessPort,'TCP','REALITY · XTLS Vision']].map(([id,name,letter,port,network,note]) => `<article class="protocol ${id}"><div class="proto-top"><h3>${name}</h3><span class="proto-letter">${letter}</span></div><div class="proto-port mono">:${port}<small>${network}</small></div><div class="muted">${note}</div></article>`).join('')}</div>`;
}
function usersView() {
...
async function showBundle(id) {
  ...
  modal(`Подключения · ${u.name}`, `${model.status.synced && !model.status.warnings.length ? '<div class="notice good">✓ Конфигурация применена на сервере.</div>' : '<div class="notice">Комплект подготовлен. Сервер ещё не подтвердил применение; файлы могут содержать тестовый домен.</div>'}<div class="bundle-actions"><a class="button-link" href="/api/users/${id}/bundle?download=1" download>↓ Скачать комплект ZIP</a><button id="share">Копировать ссылку выдачи</button></div><div class="tabs" id="bundle-tabs"><button data-file="naive.json" class="active">NaiveProxy</button><button data-file="mieru.json">mieru</button><button data-file="vless.json">VLESS</button><button data-file="vless.txt">VLESS-ссылка</button></div><pre id="config-preview"></pre><button id="copy-config" class="ghost">Копировать конфиг</button><p class="form-help">Локальные SOCKS-порты: 1080 / 1081 / 1082. Пароли и ключи в комплекте — персональные.</p>`);
  let file = 'naive.json'; dialog.querySelector('#config-preview').textContent = bundle.files[file];
