...
В комплект входят `naive.json`, `mieru.json`, `vless.json`, `vless.txt` и `README.txt`.
...
| VLESS | Xray JSON и URI с REALITY + Vision | `127.0.0.1:1082` |
...
Настройка `naiveQuic=true` разрешает HTTP/3 на Caddy, но экспорт `naive.json` в этой версии всегда содержит `https://`. Готового отдельного QUIC-профиля интерфейс не выдаёт. Собственного экспорта `mieru://`, QR-кодов и клиентских подписок нет.
...
| `trident-mita` | `trident` | Постоянный каталог для записи unit не выделяет |
